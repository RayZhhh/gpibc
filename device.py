...
@cuda.jit(device=True, inline=True)
def _pixel_index_in_stack(data_size, im_w, i, j) -> int:
    pixel_row = i * im_w + j
    pixel_col = cuda.blockIdx.x * cuda.blockDim.x + cuda.threadIdx.x
...
def _pixel_value_in_stack(stack, data_size, im_w, i, j):
    return stack[_pixel_index_in_stack(data_size, im_w, i, j)]
...
@cuda.jit(device=True, inline=True)
def _pixel_value_in_conv_buffer(buffer, data_size, im_w, i, j):
    return buffer[_pixel_conv_buffer_index(data_size, im_w, i, j)]


@cuda.jit(device=True, inline=True)
def _std_res_index(top, data_size) -> int:
    col = cuda.blockIdx.x * cuda.blockDim.x + cuda.threadIdx.x
    return top * data_size + col
...
def g_std(stack, data_size, im_w, rx, ry, rh, rw, std_res, top):
    ...
    if img_index < data_size:
        ...
        for i in range(rx, rx + rh):
            for j in range(ry, ry + rw):
                avg += _pixel_value_in_stack(stack, data_size, im_w, i, j)
        avg /= rh * rw
        deviation = 0
        for i in range(rx, rx + rh):
            for j in range(ry, ry + rw):
                value = _pixel_value_in_stack(stack, data_size, im_w, i, j) - avg
    ...
    if img_index < data_size:
        for i in range(im_h):
            for j in range(im_w):
                ...
                stack[_pixel_index_in_stack(data_size, im_w, i, j)] = d
    cuda.syncthreads()
    ...
    if img_index < data_size:
        ...
        for i in range(rx + 1, rx + rh - 1):
            for j in range(ry + 1, ry + rw - 1):
                sum = 0
                sum -= _pixel_value_in_stack(stack, data_size, im_w, i, j) * 4
                sum += _pixel_value_in_stack(stack, data_size, im_w, i, j - 1)
                sum += _pixel_value_in_stack(stack, data_size, im_w, i, j + 1)
                sum += _pixel_value_in_stack(stack, data_size, im_w, i - 1, j)
                sum += _pixel_value_in_stack(stack, data_size, im_w, i + 1, j)
                sum = max(0, sum)
                sum = min(MAX_PIXEL_VALUE, sum)
                buffer[_pixel_conv_buffer_index(data_size, im_w, i, j)] = sum

        # copy the result from buffer to stack
        for i in range(rx + 1, rx + rh - 1):
            for j in range(ry + 1, ry + rw - 1):
                stack_index = _pixel_index_in_stack(data_size, im_w, i, j)
                stack[stack_index] = _pixel_value_in_conv_buffer(buffer, data_size, im_w, i, j)
    cuda.syncthreads()


@cuda.jit(device=True)
def gau1(stack, data_size, im_w, rx, ry, rh, rw, buffer):
    ...
    if img_index < data_size:
        for i in range(rx + 1, rx + rh - 1):
            for j in range(ry + 1, ry + rw - 1):
                sum = 0
                sum += _pixel_value_in_stack(stack, data_size, im_w, i - 1, j - 1)
                sum += _pixel_value_in_stack(stack, data_size, im_w, i - 1, j) * 2
                sum += _pixel_value_in_stack(stack, data_size, im_w, i - 1, j + 1)
                sum += _pixel_value_in_stack(stack, data_size, im_w, i, j - 1) * 2
                sum += _pixel_value_in_stack(stack, data_size, im_w, i, j) * 4
                sum += _pixel_value_in_stack(stack, data_size, im_w, i, j + 1) * 2
                sum += _pixel_value_in_stack(stack, data_size, im_w, i + 1, j - 1)
                sum += _pixel_value_in_stack(stack, data_size, im_w, i + 1, j) * 2
...
@cuda.jit(device=True)
def sobel_x(stack, data_size, im_w, rx, ry, rh, rw, buffer):
...
@cuda.jit(device=True)
def sobel_y(stack, data_size, im_w, rx, ry, rh, rw, buffer):
...
@cuda.jit(device=True)
def log1(stack, data_size, im_w, rx, ry, rh, rw, buffer):
    ...
    if img_index < data_size:
        for i in range(rx + 2, rx + rh - 2):
            for j in range(ry + 2, ry + rw - 2):
                sum = 0
                sum += _pixel_value_in_stack(stack, data_size, im_w, i - 2, j)
                sum += _pixel_value_in_stack(stack, data_size, im_w, i - 1, j - 1)
                sum += _pixel_value_in_stack(stack, data_size, im_w, i - 1, j) * 2
                sum += _pixel_value_in_stack(stack, data_size, im_w, i - 1, j + 1)
                sum += _pixel_value_in_stack(stack, data_size, im_w, i, j - 2)
                sum += _pixel_value_in_stack(stack, data_size, im_w, i, j - 2) * 2
                sum -= _pixel_value_in_stack(stack, data_size, im_w, i, j) * 16
                sum += _pixel_value_in_stack(stack, data_size, im_w, i, j + 1) * 2
                sum += _pixel_value_in_stack(stack, data_size, im_w, i, j + 2)
                sum += _pixel_value_in_stack(stack, data_size, im_w, i + 1, j - 1)
                sum += _pixel_value_in_stack(stack, data_size, im_w, i + 1, j) * 2
...
@cuda.jit(device=True)
def lbp(stack, data_size, im_w, rx, ry, rh, rw, buffer):
    ...
    if img_index < data_size:
        for i in range(rx + 1, rx + rh - 1):
            for j in range(ry + 1, ry + rw - 1):
                cp_value = _pixel_value_in_stack(stack, data_size, im_w, i, j)
                p_1 = _pixel_value_in_stack(stack, data_size, im_w, i - 1, j - 1)
                p_2 = _pixel_value_in_stack(stack, data_size, im_w, i - 1, j)
                p_4 = _pixel_value_in_stack(stack, data_size, im_w, i - 1, j + 1)
                p_8 = _pixel_value_in_stack(stack, data_size, im_w, i, j + 1)
                p_16 = _pixel_value_in_stack(stack, data_size, im_w, i + 1, j + 1)
                p_32 = _pixel_value_in_stack(stack, data_size, im_w, i + 1, j)
                p_64 = _pixel_value_in_stack(stack, data_size, im_w, i + 1, j - 1)
                p_128 = _pixel_value_in_stack(stack, data_size, im_w, i, j - 1)
                sum = 0
...
@cuda.jit(device=True)
def hist_eq(stack, data_size, im_w, rx, ry, rh, rw, hist_buffer):
    ...
    if img_index < data_size:
        ...
        for i in range(rx, rx + rh):
            for j in range(ry, ry + rw):
                pixel_value = int(_pixel_value_in_stack(stack, data_size, im_w, i, j))
        ...
        for i in range(rx, rx + rh):
            for j in range(ry, ry + rw):
                raw = int(_pixel_value_in_stack(stack, data_size, im_w, i, j))
                raw = max(0, raw)
                raw = min(MAX_PIXEL_VALUE, raw)
                new_value = _hist_buffer_value(hist_buffer, data_size, raw)
                stack[_pixel_index_in_stack(data_size, im_w, i, j)] = new_value
    cuda.syncthreads()


@cuda.jit()
def calc_fit(ntype, rx, ry, rh, rw, plen, img_h, img_w, data_size, dataset, stack, conv_buffer, hist_buffer, std_res):
    top = 0
    reg_x, reg_y, reg_h, reg_w = 0, 0, 0, 0
    # reverse iteration
    for i in range(plen - 1, -1, -1):
        if ntype[i] == fset.Region_R:
            reg_x, reg_y, reg_h, reg_w = rx[i], ry[i], rh[i], rw[i]
            region(dataset, data_size, img_h, img_w, stack)
        elif ntype[i] == fset.Region_S:
            reg_x, reg_y, reg_h, reg_w = rx[i], ry[i], rh[i], rw[i]
            region(dataset, data_size, img_h, img_w, stack)
        elif ntype[i] == fset.G_Std:
            g_std(stack, data_size, img_w, reg_x, reg_y, reg_h, reg_w, std_res, top)
            top += 1
        elif ntype[i] == fset.Hist_Eq:
            hist_eq(stack, data_size, img_w, reg_x, reg_y, reg_h, reg_w, hist_buffer)
        elif ntype[i] == fset.Gau1:
            gau1(stack, data_size, img_w, reg_x, reg_y, reg_h, reg_w, conv_buffer)
            reg_x, reg_y, reg_h, reg_w = reg_x + 1, reg_y + 1, reg_h - 2, reg_w - 2
        elif ntype[i] == fset.Gau11:
            pass
        elif ntype[i] == fset.GauXY:
            pass
        elif ntype[i] == fset.Lap:
            lap(stack, data_size, img_w, reg_x, reg_y, reg_h, reg_w, conv_buffer)
            reg_x, reg_y, reg_h, reg_w = reg_x + 1, reg_y + 1, reg_h - 2, reg_w - 2
        elif ntype[i] == fset.Sobel_X:
            sobel_x(stack, data_size, img_w, reg_x, reg_y, reg_h, reg_w, conv_buffer)
            reg_x, reg_y, reg_h, reg_w = reg_x + 1, reg_y + 1, reg_h - 2, reg_w - 2
        elif ntype[i] == fset.Sobel_Y:
            sobel_y(stack, data_size, img_w, reg_x, reg_y, reg_h, reg_w, conv_buffer)
            reg_x, reg_y, reg_h, reg_w = reg_x + 1, reg_y + 1, reg_h - 2, reg_w - 2
        elif ntype[i] == fset.LoG1:
            log1(stack, data_size, img_w, reg_x, reg_y, reg_h, reg_w, conv_buffer)
            reg_x, reg_y, reg_h, reg_w = reg_x + 2, reg_y + 2, reg_h - 4, reg_w - 4
        elif ntype[i] == fset.LoG2:
            pass
        elif ntype[i] == fset.HOG:
            pass
        elif ntype[i] == fset.Sub:
